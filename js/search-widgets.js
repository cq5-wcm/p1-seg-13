...
CQ.search.Util = function() {
    ...
    return {
         ...
         */
         maskLens: function() {
            var lens = CQ.search.Util.getLens();
            if (lens) {
                var ele = lens.getMaskableElement();
                if (ele) {              
                    this.loadMask = new CQ.Ext.LoadMask(ele, {
                        msg:CQ.I18n.getMessage("Loading content...")
                    });
                }         
...
CQ.search.Lens = CQ.Ext.extend(CQ.Ext.Panel, {
    ...
    /**
     * Returns the maskable element for the lens.
     * @return {Object} data
     */
...
//                }
    ...
    loadData: function(data) {
        if (this.dataView) {
            CQ.search.Util.unmaskLens();
            this.dataView.store.loadData(data);           
         }
    },
    
    getMaskableElement: function() {
        var lensdeck = this.findParentByType("lensdeck");
        return lensdeck.getId();
    },
